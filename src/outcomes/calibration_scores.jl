...
# src/figures/05_plot_reefs_comparison.jl's reef selection exactly (low/negative ΔRMSE = model
...
# `manuscript_metrics_data.jl` is shared with src/outcomes/single_run_timing.jl, which
# writes its own "single_run_*" keys into the same file - merge onto whatever is already
# there instead of overwriting it outright, so re-running one script doesn't erase the
# other's most recent numbers.
...
merged_scores = merge(existing_scores, scores)
