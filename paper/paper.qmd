...
|  |  |
|:---:|---:|
| $$w_{t;g,z,l}^{-} ≔ \frac{C_{t;g,z - 1,l}}{C_{t;g,z - 1,l} + C_{t;g,z,l}} \cdot \frac{\omega_{t;g,z - 1,l}}{\omega_{t;g,z - 1,l} + \omega_{t;g,z,l}}$$ $$w_{t;g,z,l}^{+} ≔ \frac{C_{t;g,z,l}}{C_{t;g,z - 1,l} + C_{t;g,z,l}} \cdot \frac{\omega_{t;g,z,l}}{\omega_{t;g,z - 1,l} + \omega_{t;g,z,l}}$$ $$\mu_{t;g,z,l}^{growth} ≔ \min\left( \frac{\mu_{t;\ g,z - 1,l} \cdot w_{t;g,z,l}^{-} + \mu_{t;g,\ z,l} \cdot w_{t;g,z,l}^{+}}{w_{t;g,z,l}^{-} + w_{t;g,z,l}^{+}},\ f_{g,z,l}^{+} \right)$$ | (17) |
...
|  |  |
|:---:|---:|
| $${\mu_{t;g,1,l}^{reprod.} = \min\left( \sum_{l'}{\frac{P_{l',l}}{\sum_{l''}P_{l'',l}}\mu_{t;g,l'}^{settler}},\ f_{g,1,l}^{+} \right)   }{F_{t;\ g,1,l}^{reprod.} ≔ TN(\mu_{t;g,1,l}^{reprod.},\ \sigma_{g},\ 1,\ f_{g,1,l}^{+})}$$ | (20) |

: {tbl-colwidths="[90,10]"}

where the heritability coefficient $h^{2}$ is assumed to have a nominal value of 0.3. Because the heritable shift in equation (19) can push $\mu_{t;g,l'}^{settler}$ above the source location's own ceiling, and the connectivity-weighted combination in equation (20) can compound this across source locations, $\mu_{t;g,1,l}^{reprod.}$ is capped at the sink size class's ceiling $f_{g,1,l}^{+}$ before being used as the mean of the truncated normal.
...
+----------------------------------------------------------------------------------------+----------------+
|$$Err_{reef} = \frac{\sum_{t:\ N_{t} > 0}{w_{t} \cdot Err_{t}}}{\sum_{t:\ N_{t} >       |\(22S\)         |
|0}w_{t}}$$ $$w_{t} = t \cdot \sqrt{N_{t}}$$                                             |                |
+----------------------------------------------------------------------------------------+----------------+
...
|\frac{1}{\left| L^{comp} \right|}\sum_{l \in L^{comp}}{CCE_{l}}$$                       |                |
+----------------------------------------------------------------------------------------+----------------+

where $CCE_{l}$ is the mean of $BC_{t,l}$ over the years at reef $l$ where all functional groups' composition was observed, and $L^{comp}$ is the set of reefs with composition data. Reefs with no year of fully observed composition data are assigned a neutral $CCE_{l}$ of 0.5 rather than being excluded from the mean.

Finally, emphasis is placed on minimizing error at the peak and trough of each reef's observed coral cover trajectory. For each reef $l$, let $t_{l}^{peak}$ and $t_{l}^{trough}$ be the years of that reef's highest and lowest observed coral cover, respectively; the error at these points is calculated using mean absolute error across all reefs $L$ with at least one observation:
...
+----------------------------------------------------------------------------------------+----------------+
|$$Err_{reef} + 0.5 \cdot Err_{peak} + 0.5 \cdot Err_{trough} + 2.0 \cdot CCE$$          |\(25S\)         |
+----------------------------------------------------------------------------------------+----------------+
