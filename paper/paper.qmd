...
![Figure 4. Observed versus modelled relative coral cover for test reefs aggregated by reef group. Shaded intervals show confidence intervals.](../figures/04_reef_groups_validation.png){width="5.9in"}
...
Standing genetic variation and potential de novo mutations, represented in the model as the upper bound relative to the population mean, allow populations to evolve to higher thresholds of heat stress tolerance, up to a limit of +10DHW from the mean [@bozecRapidlyClosingWindow2025]. Despite early concerns around potentially biasing model results towards adaptation levels that are implausibly high, it is found that the represented populations do not adapt to that degree prior to population collapse. The approach does, however, lack strong empirical validation and could under or over-estimate a population\'s capacity to adapt to climate change. While these simplifications are necessary for computational efficiency, they represent areas where future work could enhance biological realism as our ability to accurately parameterise and predict coral adaptation improves.
...
+----------------------------------------------------------------------------------------+----------------+
|$$C_{t;l} = \sum_{g = 1}^{G}{\left\lbrack C_{t;g,l}^{recruits} + \sum_{z = 1}^{Z}\left( | \(2\)          |
|C_{t;\ g,z,l}^{growth} \cdot v_{g,z,l}^{background} \cdot v_{t;\ g,z,l}^{bleaching}     |                |
|\cdot v_{t;\ g,z,l}^{cyclone/COTS} \right) \right\rbrack\ }$$                           |                |
+----------------------------------------------------------------------------------------+----------------+
...
+----------------------------------------------------------------------------------------+----------------+
|$$m_{g,z,l}^{background} = m_{g,z}^{*}\left( 1 - 0.5\Upsilon_{\mathcal{l}} \right)$$    | \(3\)          |
+----------------------------------------------------------------------------------------+----------------+
...
+----------------------------------------------------------------------------------------+----------------+
|$$C_{t;\ g,z}^{growth} = \ \sum_{b = 1\ }^{B_{gz}}{\lambda_{t;\ g,z,b}\frac{\pi}{12}\   | \(8\)          |
|\left( {\delta_{g,z,b}^{+}}^{3} - {\delta_{g,z,b}^{-}}^{3} \right)}$$                   |                |
+----------------------------------------------------------------------------------------+----------------+
...
+----------------------------------------------------------------------------------------+----------------+
|$$\log\phi_{g,z} = (\log a_{g} + b_{g}\log{({\overline{\alpha}}_{g,z}))/0.1}$$          | \(9\)          |
+----------------------------------------------------------------------------------------+----------------+
...
The coral population is conceptualized as having a truncated normal distribution of heat stress tolerance $F_{t;\ g,z,l}$ for each functional group $(g)$, size class ($z$) and location ($l$). This distribution indicates the proportion of the population that is susceptible to bleaching mortality at a given DHW value, prior to incorporating an ameliorating effect of depth. Denoting a truncated normal distribution $TN(\mu,\ \sigma,\ f_{-},\ f_{+})$, where $\mu$ and $\sigma$ are the mean and standard deviations of the underlying normal distribution, and $f_{-}$ and $f_{+}$ are the lower and upper truncation bounds, respectively, we can write $F_{t;\ g,z,l}$ as:

+----------------------------------------------------------------------------------------+----------------+
|$$F_{t;\ g,z,l} ≔ TN(\mu_{t;g,z,l},\ \sigma_{g},\ 4,\ \mu_{t;g,z,l} + 10)$$             | \(13\)         |
+----------------------------------------------------------------------------------------+----------------+

The upper bound (+10 DHW from mean tolerance) is a simplification which avoids a hard limit to adaptive capacity. The lower bound represents a lack of bleaching at less than 4 DHWs [@heronWarmingTrendsBleaching2016; @vanhooidonkQuantifyingQualityCoral2009]. The distribution shape is informed by data from [@hughest.p.kerryj.t.bairda.h.etal.GlobalWarmingTransforms2018] and analyses on the heritability of coral traits [@bairos-novakCoralAdaptationClimate2021]. For each functional group, the standard deviations ($\sigma_{g}$) are assumed to be identical for all size classes and locations, and are fixed throughout the entire model run to represent stable genetic variance [@bairos-novakCoralAdaptationClimate2021]. Table S5 shows the values used for all $\sigma_{g}$. The mean values ($\mu_{t;g,z,l}$) are initially the same for all locations but vary between functional group and size class and then updated at each timestep to account for natural adaptation. Table S4 shows the initial values adopted for all $\mu_{t = 0;g,z,l}$.

At each timestep, the proportion of the population affected by bleaching for each functional group, size class and location, denoted by $\beta_{t;g,z,l}$, is estimated based on $F_{t;g,z,l}$. The affected population is adjusted with a depth coefficient ($\eta_{l}$) that is a function of that location's depth ($h_{l}$), as in [@bairdDeclineBleachingSuggests2018]:

+----------------------------------------------------------------------------------------+----------------+
|$$\eta_{l} = \min\left( \max\left( e^{- 0.07551\  \cdot \ \left( h_{l} - 2 \right)},\ 0 | \(14\)         |
|\right),\ 1 \right)$$                                                                   |                |
+----------------------------------------------------------------------------------------+----------------+

The depth coefficient represents the protection provided by depth from heat stress (as in [@lushAnimalBreedingPlans1943]) clamped between 0 and 1, and is expressed in terms of DHWs. The minimum viable depth for all locations is assumed to be $2$ meters below surface, where values lower than 2 offer no protection [@bairdDeclineBleachingSuggests2018]. Bleaching mortality is represented by:

+----------------------------------------------------------------------------------------+----------------+
|$$m_{t;g,z,l}^{bleaching} = \left\{ \begin{array}{r}                                    | \(15\)         |
|  \beta_{t;g,z,l} \cdot \eta_{l},\ \ z > 2 \\                                           |                |
|  0,\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ z \leq 2                          |                |
|  \end{array} \right.\ $$                                                               |                |
+----------------------------------------------------------------------------------------+----------------+
...
+----------------------------------------------------------------------------------------+----------------+
|$$\beta_{t;g,z,l}\sim F_{t;g,z,l}(DHW)$$                                                | \(16\)         |
+----------------------------------------------------------------------------------------+----------------+

Bleaching mortality is not applied to juveniles as there is evidence that juveniles are less susceptible to bleaching and that juvenile mortality is well-represented by other factors included in background mortality [@alvarez-noriegaContrastingPatternsChanges2018; @speareSizedependentMortalityCorals2022].
...
Since we assume all corals from size class $z = 1$ move to $z = 2$ after a growth event, we only need to update the DHW tolerance distributions for size classes 2 to 7. The distribution means for each functional group, size class, location and timestep are given by a weighted sum of means where the weights ($w$) are informed by the proportional coral cover ($C_{t;g,z,l}$) and growth rates ($\omega_{g,z}$):

+--------------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+
| $$w_{t;g,z,l}^{-} ≔ \frac{C_{t;g,z - 1,l}}{C_{t;g,z - 1,l} + C_{t;g,z,l}} \cdot \frac{\omega_{g,z - 1}}{\omega_{g,z - 1} + \omega_{g,z}}$$ | (17) |
|                                                                                                                                            |                                |
| $$w_{t;g,z,l}^{+} ≔ \frac{C_{t;g,z,l}}{C_{t;g,z - 1,l} + C_{t;g,z,l}} \cdot \frac{\omega_{g,z}}{\omega_{g,z - 1} + \omega_{g,z}}$$         |                                |
|                                                                                                                                            |                                |
| $$\mu_{t;g,z,l}^{growth} ≔ \mu_{t;\ g,z - 1,l} \cdot w_{t;g,z,l}^{-} + \mu_{t;g,\ z,l} \cdot w_{t;g,z,l}^{+}$$                             |                                |
+============================================================================================================================================+================================+
...
+----------------------------------------------------------------------------------------+----------------+
|$$F_{t;\ g,z,l}^{growth} ≔ TN\left( \mu_{t;g,z,l}^{growth},\ \sigma_{g},\ 4,\           | \(18\)         |
|\mu_{t;g,z,l}^{growth} + 10 \right),\ z = 2,\ldots,7$$                                  |                |
+----------------------------------------------------------------------------------------+----------------+

### After reproduction distribution update

The distribution for the newest generation ($z = 1$) is determined by accounting for heritability with the Breeders' equation [@lushAnimalBreedingPlans1943; @lernerGeneticBasisSelection1958; @dziedzicHeritableVariationBleaching2019; @janeiroRobustEvolutionaryInference2017; @richardsMovingHeritabilitySearch2023]. The selection differential ($S_{t;g,l}$) described in Breeder's equation is given by the difference between the mean of the previous timesteps juveniles and a weighted sum of mean tolerances of connected populations,

+----------------------------------------------------------------------------------------+----------------+
|$$S_{t;g,l} = {\overline{\mu}}_{t;l} - {\overline{\mu}}_{t;g,1,l}$$                     | \(19\)         |
+----------------------------------------------------------------------------------------+----------------+

where ${\overline{\mu}}_{t;l}$ is the weighted mean of thermal tolerance from all source recruitment locations to location $l$ for each functional group and ${\overline{\mu}}_{t;g,1,l}$ is the mean thermal tolerance of corals for functional group $g$ and size class $z = 1$ at the location $l$ and timestep $t$. The distributions for size class $z = 1$ at timestep $t + 1$ are then updated as:

+----------------------------------------------------------------------------------------+----------------+
|$${\mu_{t;g,1,l}^{reprod.} = \mu_{t;g,1,l} + \left( S_{t;g,l} \cdot h^{2} \right)       | \(20\)         |
|  }{F_{t;\ g,1,l}^{reprod.} ≔ TN(\mu_{t;g,1,l}^{reprod.},\ \sigma_{g},\ 4,\             |                |
|\mu_{t;g,1,l}^{reprod.} + 10)}$$                                                        |                |
+----------------------------------------------------------------------------------------+----------------+
...
+----------------------------------------------------------------------------------------+----------------+
|$$F_{t + 1;g,z,l}^{\ } = TN(\mu_{t;g,z,l}^{*},\ \sigma_{g},\ 4,\ \mu_{t;g,z,l}^{*} +    | \(21\)         |
|10)$$                                                                                   |                |
+----------------------------------------------------------------------------------------+----------------+
...
Prior to the calibration, reefs were grouped based on ecological similarity and spatial proximity (Figure S1). This split was done programmatically based on the Great Barrier Reef Marine Park Authority\'s bioregions [@greatbarrierreefmarineparkauthoritygbrmpaGBRMPAMarineBioregions2025; @kerriganClassifyingBiodiversityGreat2010]. These bioregions were grouped together based on spatial proximity to allow a 75%/25% calibration-test split and comparative assessment within each reef group. Refer to the "Data split and reef groups" section of the Supplementary Materials for more details.
  ...
  ---------------------------------------------------------------------------------- --------------------------- -------------------------------------------------------------------------------------
  Rate parameter of exponential distribution of coral size for each reef group.      (0.5, 2.0)                  12 (reef groups)

  Linear extension ($\mathbf{\gamma}_{\mathbf{g,z}}$)                                +/- 10% of nominal values   35 (for each of the 5 functional groups and 7 size classes)
  ...
  Initial mean thermal tolerance ($\mathbf{\mu}_{\mathbf{t = 0;g,z,l}}\mathbf{)}$    +/- 10% of nominal values   35 (for each of the 5 functional groups and 7 size classes; same for all locations)
...
AIMS Long-Term Monitoring Program (LTMP) (<https://apps.aims.gov.au/reef-monitoring/reefs>) data was used to calibrate a subset of CoralBlox parameters [@emslieDecadesMonitoringHave2020]. LTMP manta tow observations were the target for modelled total coral cover outputs and LTMP fixed point photogrammetry community composition data was used as the target for modelled community composition during calibration and validation.
...
## Tables
  ...
  ------------------- --------------------- ---------------------- -------------------------
  ΔRMSE Calibration   0.39 \[0.22, 0.61\]   0.37 \[0.17, 0.61\]    -0.40 \[-0.62, -0. 21\]

  ΔRMSE Test          0.29 \[0.02, 0.60\]   0.24 \[-0.06, 0.57\]   -0.46 \[-0.87, -0.19\]

  SRCC Calibration    0.25 \[0.02, 0.48\]   0.28 \[0.06, 0.52\]    -0.44 \[-0.64, -0.26\]

  SRCC Test           0.31 \[0.00, 0.74\]   0.13 \[-0.29, 0.56\]   -0.48 \[-0.92, -0.21\]
  ------------------------------------------------------------------------------------------
  ...
  ---------- ------------------------------ ------------------------------
     RMSE         0.08 \[0.07, 0.09\]            0.10 \[0.08, 0.11\]

    ΔRMSE         0.01 \[0.01, 0.02\]            0.01 \[-0.01, 0.04\]

     SRCC         0.59 \[0.52, 0.67\]            0.62 \[0.48, 0.78\]
  ------------------------------------------------------------------------

  : Table S2. Bootstrapped mean performance metrics and 95% confidence intervals for calibration and test reefs.

  -----------------------------------------------------------------------------------------------------------------------------
        TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  ---- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1         2.5 to 7.5              2.5 to 75.               2.5 to 7.5              2.5 to 5.0              2.5 to 5.0

   2         7.5 to 12.5             7.5 to 12.5             7.5 to 12.5               5.0 to 75              5.0 to 7.5
  ...
  ----------------------------------------------------------------------------------------------------------------------------
       TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  --- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1           3.36                    3.99                     3.81                    5.34                    6.07

   2           3.12                    3.72                     3.83                    5.31                    6.23

   3           3.41                    3.85                     3.86                    5.03                     5.7

   4           2.97                    3.26                     3.79                    5.69                    5.68

   5           3.42                    3.67                     3.65                    5.03                    5.81

   6           3.43                     3.7                     3.65                    5.21                     6.0

   7            2.9                    3.46                     3.81                     4.6                    5.53
  ----------------------------------------------------------------------------------------------------------------------------

  : Table S4. Calibrated values for initial means ($\mathbf{\mu}_{\mathbf{t = 0}}$) of the truncated normal distributions (TN) of heat tolerance for each functional group (columns) and size class (rows), in Degree Heating Weeks (DHW).
  ...
  -----------------------------------------------------------------------------------------------------------------------------
        TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  ---- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1            0.34                    0.22                     0.21                    0.23                    0.29

   2            0.21                    0.13                     0.13                    0.08                    0.08

   3            0.21                    0.09                     0.07                    0.05                    0.04

   4            0.20                    0.12                     0.09                    0.03                    0.03

   5            0.17                    0.12                     0.09                    0.01                    0.01

   6            0.14                    0.11                     0.08                    0.01                    0.02

   7            0.13                    0.10                     0.08                    0.03                    0.03
  -----------------------------------------------------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------------------------------------------------------
        TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  ---- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1            2.24                    2.43                     1.73                    1.25                     1.2

   2            4.57                    2.78                     1.58                    0.79                    0.78

   3            4.55                    2.91                     1.61                    0.74                     0.7

   4            5.85                    2.96                     1.72                    0.96                    0.95

   5            6.39                    2.93                     1.58                    1.38                    1.32

   6            8.49                    3.29                     1.52                    1.36                    1.49
  ...
  -----------------------------------------------------------------------------------------------------------------------------
        TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  ---- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1            0.89                    0.95                     1.49                    0.78                    0.75

   2            1.01                    1.28                     0.8                     0.98                    1.05

   3            0.92                    1.47                     1.04                    1.26                    1.35

   4             0.9                    0.74                     1.01                    1.31                    1.49

   5            0.88                    0.81                     0.84                    0.92                     1.0

   6            1.01                    0.71                     0.88                    0.89                    0.82

   7            0.97                    1.23                     1.28                    1.16                    0.73

   8            0.91                    0.83                     0.86                    1.26                    1.06

   9            0.87                    0.87                     0.81                    0.86                    0.84

   10           1.03                    1.22                     0.81                    0.93                    0.77

   11           1.04                    0.85                     0.74                    0.91                    0.99

   12            1.0                    1.05                     0.94                     1.0                    0.84
  -----------------------------------------------------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------------------------------------------------------
        TA ($\mathbf{g = 1}$)   CA ($\mathbf{g = 2}$)   NAC ($\mathbf{g = 3}$)   SM ($\mathbf{g = 4}$)   LM ($\mathbf{g = 5}$)
  ---- ----------------------- ----------------------- ------------------------ ----------------------- -----------------------
   1            0.18                    -0.16                    0.76                     0.6                    0.29

   2            0.52                    -0.56                    0.59                    0.12                    -0.39

   3            0.97                    0.99                    -0.41                    0.36                    0.23

   4            -0.62                   -0.27                    0.05                    -0.64                   -0.05

   5            -0.44                   -0.76                    0.24                    -0.92                   -0.01

   6            -0.94                   -0.89                    0.33                    0.83                    -0.59

   7            -0.2                    0.25                     -0.8                     0.8                    0.31

   8            -0.61                   0.71                    -0.56                    -0.2                    0.98

   9            -0.03                   0.29                    -0.26                    -0.29                   -0.87

   10           0.75                    0.27                     0.46                    0.55                    0.47

   11           -0.78                   -0.37                    -0.3                    -0.62                   -0.82

   12           0.61                    0.16                    -0.41                    -0.24                    0.9
  -----------------------------------------------------------------------------------------------------------------------------
...
In case 1, the bounds for the *block* after the growth event, at timestep $t$, are given by:
  ...
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  $${\lbrack\delta}_{t;g,z,b}^{-},\delta_{t;g,z,b}^{+}\rbrack = \lbrack\delta_{t - 1;g,z,b}^{-} + \omega_{g,z},\delta_{t - 1;g,z,b}^{+} + \omega_{g,z}\rbrack$$   (1S)
  --------------------------------------------------------------------------------------------------------------------------------------------------------------- --------------------------------
...
The growth rates used at each timestep are based on the linear extensions $\gamma_{g,z}$ and are rescaled by the coral growth and the spatial competition coefficients, as per Eq (7). This section presents details on these two coefficients calculations.
...
where the maximum projected cover $C^{*}$ is an upper bound to $C_{t}^{P}$. Since there are infinite possible upper bounds for $C_{t}^{P}$, we chose one that balances accuracy and computation speed.

+----------------------------------------------------------------------------------------+----------------+
|$$C^{*} = H\frac{{(d_{g_{\max},Z}^{+} + \gamma_{g_{\max},Z})}^{2}}{d_{g_{\max},Z}^{+}}\ | \(17S\)        |
|$$                                                                                      |                |
+----------------------------------------------------------------------------------------+----------------+

where $d_{g_{\max},Z}^{+}$ and $\gamma_{g_{max,}Z}$ are the upper bound and linear extension of the last size class ($z = Z$) of the functional group$\ g_{\max}\ $such that $g_{\max} = \arg{\max_{g}{(d_{g,Z}^{+} + \gamma_{g,Z})}}$.
...
+----------------------------------------------------------------------------------------+----------------+
|$${\kappa_{a} = 3\sum_{g = 1}^{G}{\sum_{z = 1}^{Z}{\lambda_{g,z}\gamma_{g,z}^{2}\left(  | \(20S\)        |
|d_{g,z}^{+} - d_{g,z}^{-} \right)}}                                                     |                |
|  }{\kappa_{b} = 3\sum_{g = 1}^{T}{\sum_{z = 1}^{S}{\lambda_{g,z}\gamma_{g,z}\left(     |                |
|\left( d_{g,z}^{+} \right)^{2} - \left( d_{g,z}^{-} \right)^{2} \right)}}               |                |
|  }{\kappa_{c} = \left( \sum_{g = 1}^{G}{\sum_{z =                                      |                |
|1}^{Z}{\lambda_{g,z}\gamma_{g,z}^{3}\left( \left( d_{g,z}^{+} \right)^{3} - \left(      |                |
|d_{g,z}^{-} \right)^{3} \right)}} \right) - \frac{12}{\pi}C_{t}^{A}}$$                  |                |
+----------------------------------------------------------------------------------------+----------------+
...
![Figure S. Convergence analysis for each DHW range and metric.](media/image9.png){width="6.268055555555556in" height="6.268055555555556in"}
  ...
  : Table S. 218 factors included in the sensitivity analysis and their respective sampling bounds.
