...
The distribution for the newest generation ($z = 1$) at each sink location $l$ is then given by the connectivity-weighted mean of the source contributions $\mu_{t;g,l'}^{\textbf{settler}}$, using the same connectivity weights $P_{l',l}$ as equation (11):
...
where $t$ is the year index (1 for the first calibration year, increasing thereafter), and reefs surveyed in the same year are assumed not to fail independently of one another, so $N_{t}$ enters as $\sqrt{N_{t}}$ rather than linearly. Years with no reporting reefs are excluded from equation (22S).
